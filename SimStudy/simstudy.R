...
fitlogFU <- matrix(NA,nrow=NREPS,ncol=4)
fitlogFW <- matrix(NA,nrow=NREPS,ncol=4)
colnames(fitCLR) <- c("betahat","betahat.se","cover","test.rej")
for(j in 1:NREPS) { # loop over simulation replicates
  if(j %% 50 == 0) print(paste0("Simulation replicate ", j))
  repeat {
    dat <- simdata(params[i,])
    if(length(unique(dat$exposure)) > 1) break # there is variation in exposure -- use dat
  }
  ...
  #-----------------------------
  # CMLE: clogitlogF() w/o augmentation (m=0) is CLR.
  dataug <- augment.logFmatched(form,dat,m=0)
  ...
  # logFU
  m1 <- ifelse(params[i,"exptype"]=="continuous",2.36, 1)
  dataug = augment.logFmatched(form,dat,m=m1) 
  ff = try({ clogitlogF(form,dataug,maxit=maxiter) }) 
  if(!inherits(ff,"try-error") && ff$iter < maxiter) {fitlogFU[j,] <- fitSummary(ff,truebeta)}
  # logFW
  m2 <- ifelse(params[i,"exptype"]=="continuous",5.62, 2)
  dataug = augment.logFmatched(form,dat,m=m2) 
  ff = try({ clogitlogF(form,dataug,maxit=maxiter) }) 
  if(!inherits(ff,"try-error") && ff$iter < maxiter) {fitlogFW[j,] <- fitSummary(ff,truebeta)}
}
    ...
    write.table(fitFirth,file=paste("SimRes/Firthres",i,sep="."),
                quote=FALSE,row.names=FALSE)
    write.table(fitlogFU,file=paste("SimRes/logFUres",i,sep="."),
                quote=FALSE,row.names=FALSE)
    write.table(fitlogFW,file=paste("SimRes/logFWres",i,sep="."),
                quote=FALSE,row.names=FALSE)
